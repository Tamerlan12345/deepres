...
if os.path.exists(frontend_dist):
    ...
    # Convert to absolute path once to handle trailing slashes consistently and optimize
    ...
    async def serve_spa(full_path: str):
        ...
        # Use commonpath to ensure the resolved safe_path is strictly within frontend_dist
        if os.path.commonpath([abs_frontend_dist, safe_path]) != abs_frontend_dist:
            return FileResponse(os.path.join(frontend_dist, "index.html"))

        # Offload sync I/O calls to thread pool to prevent blocking async event loop
        exists = await run_in_threadpool(os.path.exists, safe_path)
        if exists:

...
if os.path.exists(frontend_dist):
    ...
    async def serve_spa(full_path: str):
        ...
        try:
            # os.path.abspath resolves .. and makes the path absolute
            safe_path = os.path.abspath(os.path.join(frontend_dist, full_path))

            # Use os.path.commonpath to ensure safe_path is strictly within frontend_dist
            # Note: startswith is insufficient as `/dist2` starts with `/dist`
            base_dir = os.path.abspath(frontend_dist)
            if os.path.commonpath([base_dir, safe_path]) != base_dir:
                return FileResponse(os.path.join(frontend_dist, "index.html"))

        except ValueError:
            # Handle cases like differing drives on Windows
            return FileResponse(os.path.join(frontend_dist, "index.html"))

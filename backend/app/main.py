...
if os.path.exists(frontend_dist):
    ...
    async def serve_spa(full_path: str):
        ...
        try:
            # 🛡️ Sentinel: Prevent path traversal by securely validating boundaries with absolute paths
            common_path = os.path.commonpath([os.path.abspath(safe_path), os.path.abspath(frontend_dist)])
            if common_path != os.path.abspath(frontend_dist):
                return FileResponse(os.path.join(frontend_dist, "index.html"))
        except ValueError:
            # Handle cases where paths are on different drives (e.g., Windows)
            return FileResponse(os.path.join(frontend_dist, "index.html"))

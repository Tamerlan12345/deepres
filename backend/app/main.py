...
if os.path.exists(frontend_dist):
    ...
    async def serve_spa(full_path: str):
        ...
        try:
            absolute_frontend_dist = os.path.abspath(frontend_dist)
            safe_path_abs = os.path.abspath(safe_path)
            # Ensure the resolved path is strictly within frontend_dist boundary
            if os.path.commonpath([safe_path_abs, absolute_frontend_dist]) != absolute_frontend_dist:
                return FileResponse(os.path.join(frontend_dist, "index.html"))
        except ValueError:
            # Fallback if paths are on different drives (Windows edge case)
            return FileResponse(os.path.join(frontend_dist, "index.html"))

        # Offload synchronous file system operations to a thread pool

...
## 2026-02-24 - Path Traversal via Prefix Matching
**Vulnerability:** The static file serving catch-all route (`/{full_path:path}`) used `safe_path.startswith(frontend_dist)` to check boundaries. This allowed path traversal because `/path/to/dist-fake/secret` starts with `/path/to/dist`.
**Learning:** Checking path boundaries using string prefix matching (`startswith`) is vulnerable to directory traversal when the intended directory path forms a prefix for another directory at the same level.
**Prevention:** Always use `os.path.commonpath` on absolute paths (e.g. `os.path.abspath`) to securely verify that a resolved path resides strictly within the intended base directory. Handle `ValueError` for cross-drive paths on Windows.

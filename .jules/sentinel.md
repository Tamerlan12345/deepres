...
## 2026-02-24 - Path Traversal Vulnerability via `startswith` Check
**Vulnerability:** The static file serving catch-all route used `safe_path.startswith(frontend_dist)` to validate paths, which is bypassable if a requested directory has the frontend_dist as a prefix (e.g. `frontend/dist2/secret.txt` starts with `frontend/dist`).
**Learning:** Checking string prefixes for file paths is insecure. Even if paths are normalized, partial matches (like `dist2` vs `dist`) can allow reading files outside the intended directory.
**Prevention:** Use `os.path.commonpath([base_dir, resolved_path]) == base_dir` to ensure the resolved path strictly falls within the expected directory boundaries.

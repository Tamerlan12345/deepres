...
## 2026-06-23 - Path Traversal via startswith()
**Vulnerability:** Path traversal vulnerability due to using `startswith()` string prefix check for path validation in catch-all static file route. Allowed bypassing `/app/frontend/dist` using paths like `../dist-secret/`.
**Learning:** `os.path.commonpath` must be used for path boundary validation instead of simple string prefix checking, because prefix checks don't respect directory boundaries.
**Prevention:** Always use `os.path.commonpath` with absolute paths, and catch `ValueError` to handle boundary crossing safely.

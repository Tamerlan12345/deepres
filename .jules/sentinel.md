...
## 2026-02-24 - Cross-Site Scripting (XSS) in MermaidChart component
**Vulnerability:** The MermaidChart component rendered SVG generated from user-provided Markdown directly into the DOM using `dangerouslySetInnerHTML` without proper sanitization. The Mermaid configuration also had `securityLevel: "loose"`.
**Learning:** `dangerouslySetInnerHTML` should never be used with unsanitized user inputs. Even SVG strings generated by libraries like Mermaid can contain malicious scripts, requiring validation/sanitization.
**Prevention:** Always use a robust DOM sanitizer, like `DOMPurify`, when dealing with user inputs, regardless of intermediate processing steps. Additionally, set library configurations to be as secure as possible, e.g., `mermaid.initialize({ securityLevel: "strict" })`.

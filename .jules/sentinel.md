...
\n## 2026-02-24 - Partial-Directory Path Traversal\n**Vulnerability:** The SPA route used `.startswith()` to validate directory boundaries, allowing attackers to access sibling directories sharing the same prefix (e.g., `/app/frontend/dist_secret` bypassed the check for `/app/frontend/dist`).\n**Learning:** String prefix checks are unsafe for path boundary validation because they do not understand directory boundaries.\n**Prevention:** Always use `os.path.commonpath()` on absolute paths and safely catch `ValueError` to properly validate that a path resides strictly within a target directory.

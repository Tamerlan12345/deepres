...
## 2026-02-25 - Path Traversal via string startswith validation
**Vulnerability:** The SPA file server used `startswith(frontend_dist)` to validate paths. This allowed path traversal to sibling directories sharing the same prefix (e.g., `frontend/dist_secret`).
**Learning:** String-based prefix checks are fundamentally flawed for path validation because they do not understand directory boundaries.
**Prevention:** Always use boundary-aware functions like `os.path.commonpath` with absolute paths (`os.path.abspath`) to enforce that a resolved path is strictly within the intended directory hierarchy.
